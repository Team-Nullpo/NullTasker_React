...
async function migrateTickets() {
  ...
  try {
    ...
    try {
      ...
      console.log(`✓ tickets.json を読み込みました (${ticketsData.length}件のチケット)\n`);
    } catch (error) {
      if (error.code === 'ENOENT') {
        console.log('⚠ tickets.json が見つかりません。空のデータベースを作成します。\n');
        ticketsData = [];
      } else {
        throw error;
      }
    ...
    console.log(`現在のデータベース内のチケット数: ${existingTickets.length}件\n`);

    if (existingTickets.length > 0) {
      console.log('⚠ データベースに既存のチケットが存在します。');
      console.log('移行を続行すると、重複したチケットが作成される可能性があります。\n');
    }
    ...
    for (const ticket of ticketsData) {
      try {
        ...
        const normalizedTicket = {
          ...
          estimated_hours: parseFloat(ticket.estimated_hours) || parseFloat(ticket.estimatedHours) || 0,
          actual_hours: parseFloat(ticket.actual_hours) || parseFloat(ticket.actualHours) || 0,
        ...
        errorCount++;
        console.error(`✗ チケット ${ticket.id} の移行に失敗しました:`, error.message);
      }
    }
  ...
  (async () => {
    try {
      if (command === 'migrate') {
        ...
        console.log('  node migrate.js export   - SQLiteからJSONへエクスポート');
      }
    } catch (error) {
